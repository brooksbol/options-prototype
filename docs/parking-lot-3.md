...
**Date:** September 1, 2026 (intake); **remediation closure appended 2026-09-09**
**Method:** `docs/foundations/idea-intake-reconciliation.md`
**Reconciliation state:** REMEDIATED / CLOSED. Intake + disposition reconciled 2026-09-01 (independently verified by Codex); repair shipped and live-verified 2026-09-01 (`c5df959`). See "Remediation closure" below.
...
~~**Documentation only (this record + intake + journal). No implementation, schema change, runtime change, re-emission, or backfill is authorized.** Remediation design (explicit HTTP-boundary contract; end-to-end validation; provenance boundary) may be proposed for separate Principal authorization; commit remains separately gated.~~ **Superseded 2026-09-09 — remediation was authorized, shipped, and verified live on 2026-09-01. See "Remediation closure" below.**

### Remediation closure (appended 2026-09-09; discovered during the authorized 3AM truth-preflight)

**This defect is repaired.** A 2026-09-09 truth-preflight (authorized to *implement* the repair) found the repair had **already landed on `main` on 2026-09-01** and stopped rather than re-implementing a present fix. This closure record reconciles the stale record above with durable code + journal truth; it introduces no new strategic priority.

- **Repair commit:** `c5df959` — `fix(PL-DEPLOY-02-DEF01): preserve winner economics across opportunity-history HTTP boundary` (backend-only; frontend nested shape unchanged).
- **What shipped (matches the "Remediation shape" proposed above):** `OpportunityHistoryController.SurfaceObs` now carries a nested `WinnerDto` whose field names match the frontend `WinnerEconomics` exactly (`delta`/`strike`/`mid`/`spreadPercent`/`openInterest`/`volume`/`yieldAnnualized`/`posture`), mapped into the existing flat persistence columns (DB stays flat). The **ingestion invariant** is enforced with a chosen mechanism: a winner-required state (`QUALIFIED_ACTIONABLE`, `QUALIFIED_EDGE`, `EVALUATED_WAIT`, `EVALUATED_WIDE_SPREAD`) carrying a missing/incomplete `winner` is **rejected (HTTP 422)** and made observable (counter + WARN log), never silently stored economics-empty. `SqliteEvidenceStore.getSurfaceObservation` added as a bounded read-back helper.
- **Tests:** `OpportunityHistoryBoundaryTest` (233 lines) POSTs real frontend-shaped nested-winner JSON through the controller (MockMvc) and reads rows back via the store — actionable/edge/wait/wide-spread persist all eight economics fields; non-winner persists null; winner-required-missing and winner-required-incomplete are rejected and not stored; duplicate emission is idempotent and preserves economics. **Re-verified green at SYNC SHA `addd498` during this preflight.**
- **Clean evidence epoch (unchanged from the ratified boundary):** no backfill was performed; exact historical winners are not reconstructable. Pre-`c5df959` rows remain **partially valid** (state / evaluated-vs-not / chain identity / `policy_version` / `session_posture` / `strategy` / `symbol` / `expiration` / `dte` usable; winner economics NOT usable). The economics-analysis provenance boundary is the **first validated post-fix emission** after the `c5df959` deploy (backend restarted PID 42874 → 51965 onto `c5df959`; see journal 2026-09-01 "PL-DEPLOY-02-DEF01 repaired, deployed, verified live"). Never blur pre-fix and post-fix history for economics analysis.
- **Still separate / still standing:** the **browser-only emission** continuity limitation (accumulates only while a browser tab runs Decision) is architecturally expected under B-1 and is **not** closed by this defect's remediation. It remains under `PL-EVID-01` + `PL-ARCH-06`.
- **Non-implication:** closing this defect does **not** promote `LVT-BET-OUTCOME-LEARNING` (legacy L1) or the `accumulate → analyze → govern` sequence to next product priority. It removes a truth blocker; sequencing remains a separate Principal decision.
- **Rich why-state:** journal 2026-09-01 entries "Opportunity-history economics-preservation defect (PL-DEPLOY-02-DEF01)" and "PL-DEPLOY-02-DEF01 repaired, deployed, verified live; economics-valid boundary established."
...
| **P1** — Historical Lab / Spike Deletion | **COMPLETE / ACCEPTED** | Implementation commit `475ae1a` (subject `refactor(labs): remove superseded historical surfaces`). All eight superseded historical surfaces deleted; `/labs` route and historical `App.tsx` host removed (no surviving responsibility); exclusive support removed only under reverse-dependency evidence; surviving engineering/governance/operator/evidence/portfolio/recommendation/accounting/cache/provenance capability preserved. `/engineering/*` still exactly the three P2 instruments. Verification: build passed, 99 files / 1,314 tests passed, focused router/universe/engineering 12/12, changed-file lint clean, `git diff --check` clean. See journal 2026-09-04 P1 completion record. |
| **P4** — Vocabulary / Coherence | **COMPLETE / ACCEPTED** | Implementation commit `51ee08f` (subject `refactor(cleanup): reconcile deployment vocabulary and ownership`). Public route/component vocabulary aligned to Deployment (`WriteDesk` exports `Deployment`; route `"deployment"`); `/app/write` retained; stable persistence/cache/evidence/provenance identifiers retained (`"tradier"`, ETag `gen-*`); `scan-orchestrator.ts` removed and live candidate/governance result types moved to `candidate-types.ts`; obsolete scan-era declarations removed; `CsvImportLab.tsx` renamed to `CsvDiagnostics.tsx`; bounded Lab/CSS residue reconciled (`write-desk.css` reduced; `/labs` terminology pruned). No mechanical `wd-*` rename campaign. See journal 2026-09-04 P4 completion / PL-CLEANUP closure record. |

**PL-CLEANUP — COMPLETE / ACCEPTED (2026-09-04).** All four packages are COMPLETE/ACCEPTED and the ratified exit criterion is satisfied (verified below). Accepted-`main` closure SHA `51ee08f757ee28d742fc0715adf29a8b838ac6e8`. The cleanup increment is closed; Wheelwright now reassesses readiness for the next product/infrastructure expansion. GitHub Issues #9 and #10 and Kreature remain explicitly outside PL-CLEANUP.

**Exit-criterion verification (against `51ee08f`):**

- Only the three approved engineering capabilities remain behind the engineering boundary — **met** (Universe Inspection, CSV Diagnostics, Scenario Replay; confirmed in running code and smoke test).
- Governance evaluation no longer depends on legacy browser acquisition — **met** (P3; `evaluateSymbolAdmission` consumes explicit evidence; zero `/api/market` references).
- The eight superseded Lab/spike surfaces and their exclusive dependencies are gone — **met** (P1; surfaces + `/labs` route + historical `App.tsx` host absent).
- `/api/market` and obsolete direct-provider coupling no longer enlarge ordinary change scope — **met** (backend returns 404 for `/api/market`; zero frontend references).
- Operator behavior remains stable — **met** (Principal browser smoke-test PASS; Kiro runtime/API/topology verification PASS WITH KNOWN EXISTING DEFECT — the out-of-scope Kreature/Issue #10 render defect only).
