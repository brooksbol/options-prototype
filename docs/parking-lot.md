...
**Last reviewed:** August 20, 2026 (PL-EVID-07 added from multi-expiration investigation; Console top-panel/sidebar reconciliation — PL-PROD-VALUE refined with Portfolio Capital findings and Console trajectory-chart consumer)
...
| ID | Name | Summary | Concept Home |
|---|---|---|---|
| `PL-EVID-07` | Multi-Expiration / Weekly-Aware Evidence Acquisition | **Current behavior:** Wheelwright acquires one primary expiration per symbol, selected nearest approximately 21 DTE within the 7–45 admissible range. Only that expiration's chain is stored and delivered to the recommendation engine. **Current consequence:** the 7–45 DTE policy control behaves primarily as an eligibility gate on already-sampled evidence rather than as a true recommendation search space. **Verified market structure (Aug 20, 2026):** of 961 current expiration-list records (actual Tradier `/markets/options/expirations` responses, not inference from cached chains), 64 symbols expose multiple eligible expirations within 7–45 DTE while approximately 890 expose only one. **Opportunity hypothesis:** weekly-capable symbols may contain materially better put, covered-call, or buy-write opportunities at non-primary expirations. **Evidence status:** preliminary historical-chain comparisons establish that the question is worth investigating, but do not quantify opportunity cost because those chains were captured at different times and shorter DTEs are mechanically amplified by annualization. **Candidate future direction:** acquire all eligible expirations for weekly-capable symbols while leaving monthly-only symbols effectively unchanged. **Architectural attraction:** market structure naturally limits the fan-out; the database schema (PK = symbol, evidence_type, expiration) and recommendation engines (already iterate all eligible expirations looking for cached chains) appear capable of representing multiple expirations without structural change. **Known cost:** approximately +370 chains across the current universe; negligible storage impact (~1.8 MB estimated), but meaningful scheduler/freshness implications — current full-universe refresh is approximately 5 hours; additional paced requests are estimated at roughly 20–25 minutes of calendar time, and Class A weekly-symbol refreshes would become substantially heavier (~9 sec per symbol instead of ~1.1 sec). **Validation still required before implementation:** a same-session, contemporaneous acquisition experiment comparing primary-only versus all eligible expirations using the same recommendation policy and reporting actual dollars as well as annualized metrics. | `docs/21-primary-expiration-investigation.md` |
...
| `PL-PROD-VALUE` | Portfolio Capital / Operating Value | Single authoritative Wheelwright portfolio-value anchor. Experiential evidence (August 2026): "I don't know my total portfolio value in Wheelwright." **Console reconciliation (August 20, 2026)** confirmed a concrete future consumer: the Console top-region trajectory chart (point-in-time headline + time-series history of the same quantity). **Accounting-definition invariant:** the current headline number and the historical trajectory must use identical accounting definitions — these are the same primitive viewed at a point vs over time. **Name evolution:** "Portfolio Capital" may be a better name than "Portfolio Operating Value" — it is a stock quantity (how much capital does this portfolio contain?) rather than a derived operating measure. **Important distinctions — do not collapse:** Portfolio Capital (stock) vs Production (flow) vs Deployable/Eligible Capital (capacity/state) vs Encumbered Capital (commitment/state) vs Mechanical Consequence (conditional outcome) vs Fidelity Account Value (broker reconciliation). **T-bills:** Working hypothesis favors including Treasury assets in the broad capital quantity without requiring per-instrument purpose bookkeeping; inclusion does not imply deployability. **State-transition stability (working hypothesis):** Capital moving between forms within the boundary (cash ↔ shares, cash ↔ T-bills, free ↔ encumbered) should not manufacture artificial trajectory jumps if both sides belong to the canonical quantity. Economic change and capital flows may move the trajectory; mere state transformation should not. **Open short-option MTM:** Unresolved — working hypothesis favors treating open short-option MTM as obligation/economic-state rather than reducing the primary capital stock quantity. Requires deliberate reconciliation against Fidelity accounting evidence. **Premium:** Once received, premium is already part of the asset stock; must not be double-counted against Portfolio Capital. **Denominators:** Portfolio Capital must not automatically become the denominator for every percentage — different questions (whole-portfolio productivity, participating-capital efficiency, deployed-capital measures) may legitimately use different denominators. Production growth rate is a different concept entirely. **Fidelity reconciliation:** Desirable property — the difference between Wheelwright's canonical quantity and Fidelity Account Value should be explainable rather than mysterious. Whether this is a deterministic/testable invariant remains open. **Asset boundary:** Wheelwright's boundary may differ from Fidelity's account boundary; the precise boundary remains unresolved. Do not assume simple "include strategy / exclude non-strategy" — T-bills demonstrate this is too simplistic. Unresolved: precise boundary definition, exact T-bill treatment, short-option MTM treatment, historical observation persistence mechanics, mission/envelope computation (Situation Architecture). | Situation Architecture (Eligible AUM), Console Architecture (trajectory region), ADR-014, Journal 2026-08-20 |
