...
| `PL-DEPLOY-03` | Cross-Entry Multi-Path Opportunity Awareness | Observation | The cross-entry lens can surface the same underlying through multiple valid entry mechanisms simultaneously (e.g., REMX as both CSP and Buy-Write). These are not duplicate recommendations; they are distinct economic propositions with different capital use, downside exposure, premium behavior, and upside participation. Future exploration may consider making multi-path eligibility explicit at the symbol level or supporting direct side-by-side comparison, while preserving strategy-specific consequence math and avoiding forced collapse into a single symbol score or universal "best strategy." No implementation implied yet. Current behavior is valid and already exposes the phenomenon visually via the cross-entry table and the stable drawer layout. | Observed in live session 2026-08-11 |
