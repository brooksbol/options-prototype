...
| `PL-PROD-EVENTS` | Production Economic Event Ledger | Operator-facing chronological ledger of economically meaningful events explaining how the month became its current economic state. Explicitly not a raw Fidelity transaction table — broker transactions are source evidence/provenance. Event semantics: premium received, expiration, assignment, call-away, realized appreciation, capital erosion, distributions. **Thin-slice experiment (Aug 2026):** current-month view implemented using existing `AssessedTransaction[]` from backend `ProductionAssessor` — no new backend architecture, no persistence, no event store. Derivation is purely presentational from existing authoritative evidence. **Lifecycle boundary:** richer compound events (e.g., "put sold → assigned → covered call → called away" as one narrative) depend on lifecycle reconstruction already represented under PL-PORT-02; this item does not duplicate or prematurely resolve that work. The experiment tests whether individual event semantics serve the operator need without lifecycle grouping. | ADR-014, PL-PORT-02 (lifecycle dependency), `/app/production` |
