...
**Last reviewed:** August 18, 2026 (B2 experiential-evidence reconciliation — PL-PROD-FORECAST enriched, PL-PROD-VALUE created, PL-PORT-02 deferred findings absorbed)
...
| `PL-PROD-FORECAST` | Production Forecasting Capability | Wheelwright currently has no governed model for predicting month-end production beyond known/booked amounts. **Primary near-term implementation objective (August 2026).** Operator question articulated through live use: "How am I tracking this month?" — an intra-month cash-flow planning question in Bridge regime, not a prediction request. Accuracy over precision: approximate-to-nearest-$1,000 may be entirely adequate if it better reflects uncertainty. No expectation of perfect forecasting — markets move, positions resolve differently, unexpected events occur. Policy over Prediction / ADR-014 invariants preserved: resolving capital is not forecast production; no automatic conversion of capital to assumed yield; no fabricated future income. Possible governed inputs to evaluate during design: deterministic scheduled income, conditional mechanical consequences (existing Economic Consequence data), known resolution structure, deployable cash, governed redeployment assumptions. "Resolving this month" must return as an input concept during Forecast design. Each component requires explicit architectural ratification before inclusion. | ADR-014 |
| `PL-PROD-VALUE` | Portfolio Operating Value | Single authoritative Wheelwright portfolio-value anchor: capital currently participating in the options strategy operating regime. Experiential evidence (August 2026): "I don't know my total portfolio value in Wheelwright." Likely first-order scope: Wheelwright cash + current market value of Wheelwright-owned shares. Explicitly excludes unrelated brokerage holdings (e.g., T-bills held for non-strategy purposes) — Fidelity's account boundary does not automatically define Wheelwright's operating-domain boundary. Reconciles with: Eligible AUM (Situation Architecture / Bridge Income), Console NAV / Mission Progress region (architecturally reserved, unresolved), totalAccountValue (broker-reported whole-account — serves as available input, not as the concept itself). Unresolved: precise boundary definition (which assets participate?), encumbrance treatment, relationship to capacity/deployable concepts, whether historical observation capture should begin immediately. Initial implementation scope includes rudimentary historical observation if authoritative data permits, or honest capture-from-now if history does not exist. | Situation Architecture (Eligible AUM), Console Architecture (NAV region), ADR-014 |
| `PL-EVID-01` | Historical Evidence / Observation Architecture | Architecture for accumulating, storing, and analyzing observations over time. Must distinguish: market evidence history (Evidence Engine), portfolio observation history, Decision/recommendation history (ownership unresolved per reconciliation D-04), reconstructed lifecycle evidence, simulation artifacts. Foundational dependency for lifecycle assessment, opportunity observation, Level 3 models. The opportunity observation experiment (PL-DEPLOY-02) is the most actionable near-term consumer of this architecture; its local experimental form can proceed independently, but durable observation ownership remains dependent on this domain model. | `foundations/policy-over-prediction.md` (guardrail), Historical/Observational family |
