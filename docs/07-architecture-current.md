...
- **Browser-local persistent caches must remain bounded, disposable, and reconstructible.** IndexedDB or other persistent browser storage used for performance (e.g., cached chain evidence for recommendation computation) must be fully reconstructible from backend authority. Its corruption, loss, or clearing must not render core operator workflows unusable. A consumer updating a durable derived cache from an authoritative snapshot must reconcile both presence and absence — updating only what is present while retaining obsolete state does not produce a faithful cache.
...
- **Computation placement.** Server-side computation is preferred for heavy, authoritative, reusable domain computation (production accounting, lifecycle assessment, governance evaluation). Frontend computation is appropriate for presentation-oriented derivations (recommendation ranking from cached evidence, UI-local formatting, interaction-local calculations). Existing browser-local recommendation placement is transitional where already documented as such (see PL-ARCH-06). Computation whose lifecycle should be independent of the currently mounted surface should not be owned by that surface.
- **Snapshot/evidence-delivery contracts are frozen interfaces.** The snapshot representation and any derived cache-maintenance behavior constitute an existing contract. Removing or nullifying snapshot fields causes non-obvious downstream state corruption in persistent browser caches. Changes require explicit version transitions per contract documentation.
- **Experimental surfaces must fail closed relative to core operator workflows.** Optional or laboratory surfaces (e.g., Kreature) may consume Evidence, portfolio state, and other application-scoped data. A failure in an experimental surface must not impair Console, Deployment, acquisition, or maintained evidence. Pressure discovered by an experimental consumer does not authorize changing its upstream producer contract — that is an architectural finding requiring explicit resolution.
- **Consumer-path authority precedence (ADR-017).** A domain fact or verdict has one authoritative owner. Presentation/consumer layers compose and present authoritative facts; they must not reconstruct backend domain policy (e.g. session/admissibility semantics) from lower-level signals such as provider identity or cache timestamps. Publishing authority is insufficient — the actual decision consumer must consume it; an authoritative verdict outranks any local/legacy fallback, which exists only in the absence of authority and may never override an explicit verdict; and while authority is pending the consumer fails closed rather than fabricating an ordinary domain state. Cache freshness (transport) and domain admissibility (authority) are distinct concerns. See ADR-017; sibling of ADR-015 (provenance authority) and ADR-016 (association authority).
- **Brokerage lifecycle-evidence authority (clarification, September 2026).** A trade or lifecycle transition (open, buy-to-close, assignment, expiration, roll) may become portfolio/history **fact** only from **authoritative brokerage evidence**. Wheelwright may deterministically **derive** current portfolio state from that brokerage evidence, but **operator intent, broker handoff (ADR-004), recommendations, market observations, forecasts, or unsupported inference must never establish that a transition occurred.** Current binding: Fidelity CSV evidence is Wheelwright's *currently supported* brokerage-evidence mechanism (Fidelity is the present instance, not the timeless architectural concept — see broker modularity, `PL-EXEC-01`); current portfolio state is derived as **latest authoritative brokerage checkpoint (Option Summary) + subsequent brokerage-observed executed lifecycle events (Activity)**. Precedence: when a Wheelwright **projection** conflicts with newer authoritative brokerage evidence, **brokerage evidence wins** — the projection is degraded/incorrect, not co-authoritative. Projection over a *newer* brokerage checkpoint remains legitimate (the overlay contract); what is forbidden is any non-brokerage signal *originating* a lifecycle fact, or a projection *outranking* brokerage evidence. Absence of new brokerage evidence never licenses inferring that a contemplated close/assignment/expiration/roll actually happened. This is a consolidation of existing authority (ADR-004 handoff-is-not-execution; ADR-011 CSV ingestion; ADR-015/016/017 authority spine; the BUG-001 overlay contract), stated here so cold-start actors need not reconstruct it from a defect record and code; it introduces no new principle.
