...
---
## 2026-09-16 — Today's G/L broker parity: prior-close baseline (`BUG-020`)
Authorized bug-fix session. The operator compared the Console against a real Fidelity `Portfolio-Positions-Sep-16-2026` export and found the Unencumbered Shares **Today's gain/loss $/%** materially wrong — several rows off by hundreds of dollars, some with the **opposite sign** (COPX ≈ −$12 vs Fidelity +$134; SMH ≈ −$98 vs +$891). Filed as **BUG-020** (S2) and fixed the same session.
### Root cause (reconfirmed against `main` @ `1156178`)
The shared `today-gl.ts` measured "Today's G/L" as `latest − firstObservationOfLatestDay` — Wheelwright's *first intraday sample* of the day, not the prior session close. That baseline excludes the open gap (so magnitudes are far too small) and inverts sign whenever a symbol is up vs prior close but has drifted down since the first sample. Tradier's `/markets/quotes` already carries `prevclose` (proven by `getIndexQuote` for the Markets card), but the per-symbol `normalizeQuote` discarded it.
### Supersedes the Sep-15 scope note
The `PL-MKT` entry recorded a Principal decision that "Today's G/L stays on observed `spot_history`." That decision is **superseded for the Today's G/L label**: broker parity requires the prior-session close, which `spot_history` cannot supply. The sparkline SHAPE still uses timesales; the moneyness value cell still uses observed `spot_history`. Only the Today's G/L baseline changed.
### Fix — additive `previousClose`, one canonical derivation
- **Backend:** `MarketChain.Underlying` gains a nullable `previousClose` (+ back-compat 3-arg ctor). `normalizeQuote` reads `prevclose` (absence → `null`, never `0`); `getOptionsChain`/`normalizeChain` thread it; `marshalChain` emits `underlying.previousClose` via `nullableNumber`. It rides inside the existing chain blob — **no DB migration**. `SqliteEvidenceStore` reads it back (`extractUnderlyingPreviousClose`, explicit-`null`-safe) and `getQuoteObservations` returns it; `QuotesController` emits `observation.previousClose` (nullable).
- **Frontend:** `QuoteObservation.previousClose`; `observation-store` parses it; `today-gl.ts` **removes** the first-observation functions and adds the canonical `computeTodayGlPerShare` / `computeTodayGlDollar` / `computeTodayGlPercent` (`last − previousClose`; null/dash when prior close absent or non-positive). `UnencumberedInventory` (per-row, totals, and `buildUnencumberedCsvRows`) and the DTE ladder (regime-B UI row + CSV, via `MonitoredPosition.underlyingPreviousClose`) all consume the one derivation — UI and CSV agree by construction. Demo mode supplies a synthetic coherent prior close.
- **Ladder decision:** the ladder "Today's G/L" is the same underlying-daily-move semantic as the shares table, so it was fixed with the same prior-close derivation — no split defect.
### Contract
`docs/contracts/evidence-snapshot-v1.md` documents `underlying.previousClose` (and `quotes[].observation.previousClose`) as an **additive, non-breaking** field under INV-PUB-05 (no version increment); absence is `null`, never `0`; consumers must not reconstruct a prior close from `spot_history`.
### Verification
Frontend `tsc --noEmit` clean; full vitest 1589 passing (sole failure the pre-existing, unrelated velvet-rope date-drift snapshot). New guards cover sign-reversal (COPX +$134), large-gap (SMH +$891), fractional (SPYI 69.829), $/%-consistency, prevClose-unavailable→dash, and UI/CSV agreement. Full backend JUnit suite green; regression tests added across TradierAdapter, QuotesController, SnapshotController, AcquisitionWorker (marshalChain), and SpotHistory (getQuoteObservations).
**Live upstream proof (market open):** direct Tradier production `GET /markets/quotes` returned `prevclose` == Fidelity prior close **exactly** for all five rows (COPX 84.59, GDXJ 120.31, SMH 542.11, SPYI 52.6662, URA 41.77). `(last − prevclose) × qty` at the live moment reproduces broker-parity sign/magnitude (SMH +$924 vs Fidelity +$891; COPX positive, not the defect's negative) — residuals are normal real-time-vs-11:22-ET quote drift. The running appliance predates this build (stored blobs lack `previousClose`); a restart + one acquisition cycle is required before the live Console surfaces it — not performed to avoid disrupting the single acquisition authority without authorization.

### 2026-09-16 — BUG-020 accepted and closed (Principal)
After rebuild/restart + reacquisition, the live Console Unencumbered Shares Today's G/L matched Fidelity within quote-timing tolerance on non-simultaneous screenshots: COPX +$76 vs +$77 (−$1), GDXJ +$13 vs +$17, SMH +$809 vs +$785 (≈$0.24/share), URA −$12 vs −$12 (exact), SPYI +$10 (older WW quote). Total WW +$896 / +1.08% — sensible/positive, vs the old nonsensical negative total. `prevclose` flowing, Fidelity semantics confirmed. Principal accepted; BUG-020 **Resolved**. The 17–19 min ages on some rows are a separate acquisition-freshness matter, out of scope for this fix. Follow-up commit `24a56e9` (non-positive prior-close guard + end-to-end quote-parse test) shipped alongside the primary `5acf945`.

---
## 2026-09-16 — Recipes enter the pipeline as `PL-RECIPE-01` (INTAKE); the winner-selection collapse is the reason

Authorized documentation-preparation session (no code, no experiment, no commit). A 4AM discovery discussion plus a targeted reconciliation converged on Wheelwright **Recipes** — named, versioned **decision lenses** over the governed candidate population — and the Principal authorized durable intake. Recorded as `PL-RECIPE-01` in `docs/parking-lot-8.md`. This entry preserves the *why*, not the record.

### The load-bearing implementation finding (why recipes need an earlier population)
Tracing `options-prototype/src/write-desk/recommend.ts` (`recommendPuts`, SYNC `0bb91c5`) established that the pipeline **destructively collapses each symbol's contract population to a single winner** (`bestActionable` / `bestEdge` / `bestWait`) *before* final ranking and display. So "today's visible Deployment board" is already ≈ one survivor per symbol, posture-filtered and display-capped — every other admissible strike/expiration for that symbol has already been thrown away. A decision lens that only reranked that board would be choosing among pre-selected yield-winners and would be **structurally biased toward merely re-expressing yield**, which is precisely the null result the first experiment must be able to detect. That is the concrete reason the accepted population boundary is **post-admissibility, pre-winner-selection**, not "rerank the board." It is also a direct expression of AR5 (acceptability prunes; comparative fitness ranks only survivors) — today's winner-selection silently fuses acceptability and a hidden fitness function into one step.

### What is settled vs open (so a cold actor doesn't reopen settled ground or freeze open questions)
- **Settled:** the population boundary; the three-arm first experiment (accepted Wheelwright run through its *real* selection on frozen inputs, raw yield, delta-constrained yield); that **Delta Fence + yield ≡ delta-constrained yield** (an invariant/check, not a fourth arm); and the deliberately-limited claim of that first experiment (it tests one explicit constraint's effect, not competing ranking philosophies).
- **Open (preserved, not frozen):** whether recipe evaluation replaces/parameterizes/runs beside winner-selection; whether Recipes earns its own Explorer surface (strong hypothesis, aligned with cognitive-role separation, but not a UI requirement and not built); the distinct ordering preference a *future* independent recipe would need; normalization for any multi-Greek recipe; and the whole DSL question (deferred until repeated concrete recipe semantics earn it).

### Evidence-semantics guardrails carried into the record
`midIv` ≠ `smvVol` (never collapsed to generic `iv`, no local inversion); absolute IV ≠ IV richness; missing evidence stays null, never a fabricated zero or silent score penalty; Surface Agreement/Dislocation stay research diagnostics; raw weighted Greek sums are dimensionally suspect. Recipes express *intent*; the evidence/domain layer keeps owning evidence meaning (ADR-013/015/016/017).

### Epistemic status of this step
`PL-RECIPE-01` is **INTAKE with targeted questions Principal-settled** — deliberately **not** RECONCILED. Full §3 Strategic and §4 Architectural Reconciliation Completion Records, and any roadmap/architecture-roadmap dispositions they imply, remain to be produced under separate authorization; I did not stamp RECONCILED or edit `roadmap.md`/`architecture-roadmap.md`, because this authorization was scoped to intake/why-state preparation and the targeted reconciliation resolved the *targeted* questions, not the full strategic/architectural reconciliation. The paused, uncommitted Greeks/IV (`PL-DEPLOY-EXPORT` §6) working tree was not touched.

---
## 2026-09-16 — `PL-RECIPE-01` advanced to RECONCILED (§3 strategic + §4 architectural reconciliation performed)

Authorized documentation-preparation continuation (no code, no experiment, no commit within this reconciliation step; the earlier intake was already committed `b198d5e`). The prior `-3` entry recorded that full §3/§4 reconciliation "remain[ed] to be produced under separate authorization." The Principal then authorized moving `PL-RECIPE-01` through reconciliation → completion record → experiment design → implementation-readiness review, **stopping before implementation authorization**. This entry preserves the *why*; the durable record is the **Reconciliation Completion Record for `PL-RECIPE-01`** in `docs/parking-lot-8.md`.

**SYNC:** reconciliation performed at `05ada58` (accepted `main` advanced `b198d5e`→`05ada58`; the advancement was the concurrent HOLD/CLOSE thread committing its own work — immaterial to `PL-RECIPE-01`, verified it touched none of the recipe files and that `b198d5e` is an ancestor of `HEAD`).

### What was established (earned, not pre-written)
- **§3 strategic:** strengthens existing direction, **no new Bet, no `roadmap.md` edit.** The near-verbatim mapping is `LVT-INIT-COMP-COMPARE` ("Compare those measures against current ranking dimensions") — the three-arm experiment *is* that Initiative made concrete, beneath `LVT-BET-COMPENSATION`. Also strengthens `LVT-BET-ACCEPTABILITY`/`LVT-INIT-ACCEPT-GATES` (population boundary = acceptability-before-fitness) and `LVT-BET-EXPLANATION`/`LVT-INIT-EXPLAIN-TRADEOFFS` (structured disagreement). Recipes-as-plural-lenses actively supports the roadmap "no scalar Deployment score" non-goal. Outcome-learning mapping is future-only, not activated by the first experiment.
- **§4 architectural:** **refines AR5, no new engine.** Verified in `recommend.ts` `recommendPuts` that current winner-selection **implicitly fuses** acceptability and comparative fitness and **destructively collapses each symbol to one contract** (`best = bestActionable ?? bestEdge ?? bestWait`, execution-posture-first then score) *before* `rankByPolicy`. A recipe is the *explicit, general* realization of the AR5 comparative-fitness stage the collapse hard-codes. Population boundary = post-admissibility (verified gate chain incl. Issue-#16 `isSubjectAdmissible`) / pre-winner-selection. Conforms to the ADR-013/015/016/017 authority spine (recipe is a consumer; `exportGreeks` already sits in the candidate "never influences assessment/posture"). Evidence semantics stay in the evidence layer. AR4 normalization and recipe-decision-provenance are future deps; AR8 frozen-substrate discipline governs the experiment now. **No `architecture-roadmap.md` edit.**

### The load-bearing durable insight
Current winner-selection collapses the per-symbol contract population to a single winner before ranking. That is *why* a recipe must operate on the pre-collapse population, and *why* the experiment's Accepted-Wheelwright arm must run the real collapse+rank path rather than a re-sort of already-collapsed winners. Getting this wrong would silently bias the falsification experiment toward a false negative.
