...
### End of day — Production Evidence CSV (PL-PROD-EXPORT-01 V1) shipped

Today closed the Production Evidence CSV slice end to end: ADR-016 (evidence-to-domain association is an authoritative semantic claim) was ratified earlier in the arc; the Production Disposition Truth slice merged (`64d7708`); then PL-PROD-EXPORT-01 was taken from discovery → hybrid design → implementation → two adversarial trust-correction passes → live validation → commit.

**Shipped (frontend-only; no backend files touched):** one downloadable hybrid Production CSV that serializes the authoritative backend `ProductionResponse` alongside the material operator-presented `EpisodeChapter` claims, so economic truth can be tested and backend/presentation divergence (the #11 class) detected without screenshots. Trust properties earned across the correction passes: single shared chapter derivation (renderer + export); `record_layer`/`owner` vocabulary; `presentation_group_key` separated from `parent_record_key` with full referential integrity; `presentation_role` (DISPLAYED_CLAIM vs PRESENTATION_BACKING_VALUE); disposition support only on the called-away resolution chapter (`capital_amount → net_sale_proceeds`, `confidence → disposition state`), presentation lifecycle `state` de-linked, `production_amount` not blanket-supported; reversible formula-neutralization on free text only with identifiers preserved byte-for-byte and an explicit encoding-contract metadata block; `broker_run_date` named honestly; `export_generated_at` export-time only; `build_revision = NULL` (no self-contained cross-build attribution in V1).

**Live validation:** against the running servers, a real `POST /api/production/assess` (July fixture, 32 transactions) → CSV passed the structural acceptance checks (95 rows; 0 unresolved parent/support keys; encoding-contract metadata present; source_filename labeled workflow context; identifiers byte-exact). The July fixture carries no called-away dispositions, so the disposition/divergence paths (checks 4/5/6/#11) were validated through a synthetic disposition + open/resolution pair via the same serializer. A future live check with real called-away Activity data would exercise those rows in the browser.

**Committed to `main`:** `c2712bb` (feat: code + tests) and `df7a259` (docs: PL-PROD-EXPORT-01 V1 trust contract). SYNC after push: `df7a259`.
