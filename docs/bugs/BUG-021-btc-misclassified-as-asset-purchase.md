...
- **Close-chapter quantity uses the OBSERVED close quantity (Codex falsifier).** The close chapter previously took `contracts` from `episode.contracts`, which is `0` for an unmatched BTC (skeleton episode with no recognized opening) — so a genuine unmatched BTC of quantity 3 rendered as `0`, erasing known BTC quantity when association is `UNRESOLVED`. The sixth pass changed the close chapter's `contracts` to the **observed** close quantity (`CloseEvent.quantity`), falling back to `episode.contracts` only when the row carried no usable quantity. *(The fallback itself is superseded by the seventh pass — see below — because for a quantity-unavailable close it substituted the opening quantity.)*
...
**Seventh pass — the ONE Principal-authorized bounded attempt (negative-specimen fix):** executed under the durable Principal authorization recorded above (2026-09-17). One in-scope defect corrected against the negative specimen (**STO 2 → BTC quantity unavailable / −$40**):
- **Unknown BTC quantity must stay unknown; opening quantity is never substituted.** The sixth-pass close-chapter quantity `ce.quantity ?? episode.contracts` still fell back to the episode's opening quantity when the close quantity was unavailable — so an STO-2 → quantity-unknown BTC rendered its close chapter as **2** (the opening quantity substituted as the observed closing quantity). Fixed: `EpisodeChapter.contracts` is now `number | null`, and the close chapter carries **exactly** the observed close quantity `CloseEvent.quantity` with **no fallback** — `null` when unavailable (unknown stays unknown). The two consumers render null honestly: the ledger disclosure shows `× unknown`; the Production Evidence CSV emits an explicit NULL `contracts` backing value (the serializer already emits nulls). Opening/resolution/disposition chapters are unaffected (they still carry the episode quantity, a number). Executed closing cash (−$40) remains visible throughout; association remains `UNRESOLVED`; no capital-deployment treatment; no false retirement.
- **Backend↔Product path demonstrated (not unit-only).** The backend authority already emits `OptionCloseResult.closedQuantity = null`, `status = UNRESOLVED`, `executedDebit = −40` for a quantity-unavailable close (`ProductionAssessor.buildOptionCloseResult` case (c) → `unresolvedClose(..., null, ...)`); the Product now renders that verdict faithfully. A backend assertion (`negativeSpecimenUnknownBtcQuantityBackend`) and a frontend assertion (the negative-specimen close-chapter test) pin both ends of the same path.

**Corrected claims (superseded by the seventh pass):**
- ~~"Falling back to `episode.contracts` when the close quantity is unavailable is acceptable."~~ **Wrong.** For a quantity-unavailable close over a recognized opening, that substitutes the opening quantity as the observed closing quantity (BUG-021 negative-specimen violation). The close-chapter quantity is the observed close quantity or `null` (unknown) — never the opening quantity.

*(Status of the seventh pass, updated: it was submitted for independent Codex review and **REJECTED** — it manufactured an unsupported opening association. See "Principal authorization (2026-09-17, second)" above for the cross-actor evidence, and the eighth pass below for the model-based attempt that supersedes it.)*

**Eighth pass — the ONE NEW Principal-authorized bounded attempt (validated-invariant / opening-identity fix):** executed under the second durable Principal authorization recorded above (2026-09-17, second). Codex REJECTED the seventh-pass candidate and independently falsified the failure class: for the same OCC contract, Product asserted a singular opening date for a close (`opened Jul 20` / `link_date=2026-07-20` for a Jul-10 close; and, under the stronger multiple-prior-STO specimen, the asserted opening flipped between Jul 3 and Jul 7 purely on CSV row order) — an opening identity the backend `OptionCloseResult` never establishes. One in-scope frontend change corrects this against the validated invariant:

- **A close asserts no singular opening the backend did not establish.** In `episode-derivation.ts`, `buildSingleCloseChapter()` previously set `linkDate = (deterministic || OVER_CLOSE) ? episode.openDate : null` and `linkDirection: "opened"`, where `episode.openDate` is the STO date selected by *encounter order* among possibly multiple fills. That converted arbitrary row/encounter order into a semantic opening identity (order-dependent; future-opening-unsafe). Fixed: a close chapter now carries `linkDate = null` and `linkDirection = null` unconditionally. The backend authority (`OptionCloseResult`) establishes close **economics** for the contract *series* — executed debit, observed/null quantity, outstanding-before/matched/residual/excess ranges, status — but no opening row/date/fill allocation; Product therefore asserts none. Every established close economic fact remains rendered from the close row and the backend result (missing opening identity never erases established close economics).
- **The order-selected opening STO is no longer a close constituent.** `buildSingleCloseConstituents()` previously prepended `episode.openEvent` (the encounter-order STO) to the close chapter's constituent events — Codex observed the future Jul-20 STO listed as a close constituent and exported. Fixed: a close chapter's constituents are the close row itself only. Opening provenance lives on the opening's own chapter.
- **Scope held (no creep).** `buildOpenChapter` / `buildResolveChapter` / `buildDispositionChapter` are unchanged. The demonstrated and falsified failure class is the **BTC close** asserting a singular opening; an expiry/assignment is a genuine lifecycle resolution of the same contract series (not a backend-contradicted association), and the opening chapter legitimately records that an STO occurred. No lifecycle-retirement machinery restored; no association framework introduced; no In-Flight/current-position, BUG-001, or BUG-023 work; the `Closed · obligation retired` wording is unchanged. The seventh-pass unknown-quantity fix (`contracts: number | null`, no opening-quantity substitution) is preserved.

Change is **uncommitted** pending independent Codex review, then Principal acceptance (direct-`main` workflow: Principal validation is the acceptance gate; passing tests do not authorize commit). *(The authorization record above — "Principal authorization (2026-09-17, second)" — is separately committed; the implementation candidate is not.)*
...
- **Frontend test** `options-prototype/tests/production/episode-derivation-buy-to-close.test.ts` (14 cases): renders the authoritative `OptionCloseResult` (complete/partial/over-close/unresolved, encumbrance only for an exact deterministic matched qty, divergence guard, missing-cash/quantity, two dated closes, cross-month, negative control) — plus the sixth-pass cases (the **unmatched-BTC quantity falsifier**: quantity 3 / −$40 keeps its observed quantity and cash visible, `UNRESOLVED`, no capital-deployment, no retirement claim; and a **matched-DBO-preserved** regression: observed quantity 1 still renders as 1) — plus the seventh-pass **negative specimen** (STO 2 → BTC quantity unavailable / −$40: close-chapter `contracts` is `null`/unknown, never the opening `2`; cash visible; `UNRESOLVED`).
...
- **Sixth-pass verification.** Observed at SYNC `0e0a3ec` (working tree; the advance from the fifth-pass SYNC to `0e0a3ec` was documentation-only — BUG-022/023/024 filings and a merged BUG-022 write-desk fix — and touched no BUG-021 files). `tsc --noEmit` clean; `episode-derivation-buy-to-close.test.ts` 13/13; full frontend production run 98 pass; full frontend suite 1729 pass / 1 pre-existing **unrelated** failure (`tests/velvet-rope/multi-expiration.test.ts`, a date-relative snapshot). Backend BUG-021 tests unchanged (frontend-only amendment) and green.
- **Seventh-pass verification (the ONE authorized attempt).** Observed at SYNC `00bde24` (working tree). The negative-specimen falsifier was **reproduced RED first** (`STO 2 → BTC qty-unavailable` rendered the close chapter as the opening quantity `2`), then fixed. After the fix: `tsc --noEmit` clean; `episode-derivation-buy-to-close.test.ts` **14/14** (adds the negative-specimen close-chapter case: `contracts` is `null`/unknown, not `2`; `−$40` visible; `UNRESOLVED`); full frontend production **99 pass**; full frontend suite **1730 pass / 1** pre-existing unrelated failure (the same velvet-rope date-drift snapshot). Backend: new `BuyToCloseProductionTest.negativeSpecimenUnknownBtcQuantityBackend` asserts the authoritative `OptionCloseResult` has `closedQuantity == null` (opening qty **not** substituted), `status == UNRESOLVED`, `executedDebit == −40`, cash still nets into period `OPTION_PREMIUM`, and the assessment is not `FULLY_RECONCILED`; full backend suite **BUILD SUCCESSFUL** (0 failures). This demonstrates the actual backend→Product accounting path for the negative specimen, not isolated unit behavior.
- **Eighth-pass verification (the ONE NEW authorized attempt).** Observed at SYNC `b9f5e65` (working tree). The failure class was **reproduced first** (the seventh-pass close chapter asserted `linkDate = episode.openDate`), then fixed. After the fix: `tsc --noEmit` clean; `episode-derivation-buy-to-close.test.ts` **18/18** — adds four falsifiers against the validated invariant: **(A future opening)** Jul-10 BTC with a later Jul-20 STO → close chapter `linkDate`/`linkDirection` are `null`, the Jul-20 STO is not a close constituent, economics preserved; **(A end-to-end through the CSV)** the same specimen driven `deriveEpisodeChapters → buildProductionCsv` → the exported close's `link_date` and `link_relationship` claims are the explicit `NULL` token and `2026-07-20` never appears in the close's presentation group, while `production_amount`/`contracts` survive; **(B order-independence)** `Jul-3 STO 1 / Jul-7 STO 2 / Jul-10 BTC 3` renders a **byte-identical** close chapter under reversed STO input order (`toEqual`), with no order-selected opening date; **(E partial close)** matched/residual rendered from the backend result without allocating the close to a particular opening fill. Full frontend production run **75 pass** (8 files, incl. the real-backend-fixture round-trip); full frontend suite **1734 pass / 1** pre-existing **unrelated** failure — `tests/velvet-rope/multi-expiration.test.ts`, the same date-relative inline snapshot (it computes DTE from the current date; the recorded snapshot expected `2026-09-18`, the run date produces `2026-10-02`). Backend: **frontend-only change; no backend file modified** (`OptionCloseResult.java` and all production Java unchanged from the frozen candidate). Backend BUG-021 tests (`BuyToCloseProductionTest`, `Bug021RoundTripFixtureTest`) pass; the full backend suite showed a single failure in `ObserverRealPacerIntegrationTest` (provider/pacer timing) that **passes in isolation** and is unrelated to Production/BUG-021 (a flaky real-pacer integration test under full-suite concurrency), not in the candidate or this change.
- **Single-authority (no divergence).** The Product renders the backend `OptionCloseResult`; the frontend has **no** association resolver, and both `isFullyClosed()` and the interim `episodeFullyRetiredByBackend()` open-chapter state deciders have been removed. The open chapter's complete/in-flight state derives from a genuine resolution (expiry/assignment) only; the close chapter renders each executed BTC (observed debit + observed quantity) with the backend's `OptionCloseResult` status, and (eighth pass) asserts **no** singular opening relationship — the backend does not establish one.
- **Known limitation (documented, not repaired — outside the authorized/falsified failure class).** The **opening** chapter still records the opening at `episode.openDate`, which for a contract with multiple STO fills is the encounter-order fill date, and its quantity is the summed fill quantity. This is the opening event's own record, not a close→opening association, and it is *not* the demonstrated/falsified defect (which was the BTC close asserting a singular opening). A faithful per-fill opening representation would be a separate, larger concern (association/lifecycle territory explicitly excluded by this authorization) and is deliberately left unchanged.

## Principal acceptance / closure (2026-09-17)

Status advanced **Open → Resolved**. Basis:

- **Independent review:** Codex reconciled disposition **ACCEPT** at review SYNC `92bbd051401d5b84d3252e2a57f9c411b829e426`. Codex explicitly **withdrew** its earlier CSV-based REJECT because that criterion was **not** established by durable BUG-021 authority, and found no remaining durably-authorized BUG-021 requirement demonstrably failing.
- **Principal decision:** ACCEPT the frozen candidate and authorize commit/push (Principal Decision Surface option A).
- **Remediation** is the candidate recorded in "Remediation history" above (backend classification → decomposition → single-authority `OptionCloseResult`; Product renders the backend verdict; observed BTC quantity/cash preserved, unknown stays unknown). **Verification** is as recorded in "Verification" above plus the pre-commit re-run at acceptance: backend `BuyToCloseProductionTest` + `Bug021RoundTripFixtureTest` pass; frontend `tsc --noEmit` clean; frontend production suite pass.

Preserved as **separate, still-open findings** (explicitly NOT part of BUG-021's resolved scope, per Principal disposition):

1. **Production CSV does not serialize the complete authoritative `OptionCloseResult`** (ranges/status). A real, separate defect — never a BUG-021 rejection criterion. Not dispositioned here.
2. **Process finding:** an earlier `rebase --autostash` violated an explicit no-stash instruction. Preserved as a process finding; no stash/autostash used for this acceptance commit.

This record self-closes to `Resolved` only on the strength of the Principal acceptance above; the commit that persists the candidate also persists this closure record.
