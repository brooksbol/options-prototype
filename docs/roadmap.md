...
- **`LVT-VISION-WHEELWRIGHT` — Wheelwright Vision** — Wheelwright is a continuously operating options-income decision system that connects evidence, choices, and consequences—helping the operator understand what is happening, what can be done, what could happen, and what actually happened.
  - **`LVT-GOAL-AWARENESS` — Understand the Situation** *(legacy `G1`)* — Continuously maintain trustworthy understanding of relevant market, portfolio, position, and evidence state; make material changes and uncertainty apparent.
    - **`LVT-BET-OBSERVATION` — Continuous Observation** *(legacy `A1`)* — Continuous observation reveals materially useful changes that periodic operator inspection misses.
      ...
      - **`LVT-INIT-OBS-TRUST` — Evidence Trust** — Make evidence fitness for its intended reliance — and the inability to establish that fitness — explicit, including freshness, insufficiency, disagreement, and degradation.
...
- **Survival vs. insufficiency** — three states must not collapse into one another: (1) an actionable alternative clears governed eligibility and acceptability; (2) sufficient evidence establishes that no actionable alternative clears, so WAIT (`LVT-BET-WAIT`) may be the governed surviving alternative; (3) evidence is insufficient to establish the choice set at all. State (3) is not a governed WAIT result — an operator may prudently choose to wait, but Wheelwright must preserve insufficient evidence as the reason rather than manufacture a governed WAIT. This connects state (3) to `LVT-INIT-OBS-TRUST` (evidence fitness / inability to establish fitness); it does not redefine or move `LVT-BET-WAIT`, `LVT-BET-ACCEPTABILITY`, `LVT-INIT-ACCEPT-GATES`, or `LVT-INIT-ACCEPT-WAIT`.
