...
**Status:** ACTIVE RESEARCH OVERLAY — preserves post-corpus reconciliation, external-verification results, source limitations, and remaining research questions  
...
> This document preserves reconciliation and verification work performed after Practitioner Corpus v1 was frozen. It distinguishes corpus-derived observations from externally checked claims and analyst/principal synthesis. A `RESOLVED` status means the research question or tension has a disposition; it does not automatically validate a trading prescription or promote it into Wheelwright truth.
