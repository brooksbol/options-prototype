...
12. **Completion indication tracks visible-state convergence — but always bounded.** When a control's visible result is what establishes operator trust, its "done" signal must correspond to the moment the visible state has actually converged, not the moment the underlying request returned — *and* it must be impossible for the indicator to run indefinitely. Action completion and visible-state convergence are different events: a spinner tied to "the request came back" reads as "this row is now current," which is a lie if the displayed value hasn't updated yet. So keep the spinner through convergence — but only while convergence is bounded. If the visible state does not converge within an explicit time bound, terminate into an explicit unresolved/warning state (never a clean "done," never an eternal spin). Both failure modes are unacceptable and this principle rejects both: (a) the spinner stops while the displayed value is still stale, and (b) the spinner waits forever on a distributed pipeline it cannot authoritatively observe. (Learned from working software, including a near-miss reversal: the Cash Deployment per-row refresh first stopped its spinner when acquisition returned — seconds before the Age updated; the correct fix keeps it spinning through the row's provenance advancing, with an 8s bound that terminates into an explicit `?` if it cannot settle. A subsequent Codex review generalized the same discipline to the bulk "Refresh top opportunities" control — which had claimed `Refreshed N` at request return — so it too reports `Updating evidence…` until the requested rows' visible provenance advances, then `Refreshed N` / `Refreshed X of Y` / `Not confirmed fresh`; and to the completion *label*, which must not translate a backend acquisition disposition into a confirmed-freshness claim. Principle 11 is intentionally absent from accepted `main`: it belongs to the branch-only expanded-row experiment (`ui/deploy-expanded-row`) and is not promoted here until that experiment is accepted. The gap between 10 and 12 is deliberate, not an omission.)
