...
**Current status:** **Suspended as runtime execution-control mechanism pending Gate Experiment 001**  
...
During Gate Experiment 001, conversational claims are never authoritative execution state. The experiment's tested external permission boundary determines whether consequential mutation is available.

Do not create another prose contract to replace this one before the capability-boundary experiment produces behavioral evidence.