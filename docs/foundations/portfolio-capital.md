...
1. **Persistence:** How are historical Portfolio Capital observations stored? (localStorage, backend SQLite, or both). Current implementation: localStorage with a hardcoded 16-point seed from prior exports. Adequate for operator-facing trajectory; migration to backend persistence deferred per PL-PROD-VALUE.
2. ~~**T-bill securities:**~~ **Resolved.** The V1 formula uses `Fidelity Total Account Value` which Fidelity confirms includes all held securities (equities, bonds/fixed income, T-bills, money-market) in a single aggregate. Whether T-bills appear in `cashAndCredits` vs `valueOfInvestments` is irrelevant because V1 consumes the Total, not its sub-components. The earlier investigation note (Aug 20, 2026) about T-bills in "Bonds/Fixed income" → "Value of Investments" applied to a candidate component-level formula that was superseded by the aggregate approach. No coverage gap exists for any asset Fidelity includes in Total Account Value.
