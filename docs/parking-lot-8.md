...
The 4AM discovery and targeted-reconciliation discussion (this pipeline); the implementation finding in `options-prototype/src/write-desk/recommend.ts` (`recommendPuts` winner-selection collapse); `architecture-roadmap.md` AR4/AR5/AR8; `docs/18-recommendation-vocabulary-review.md`; `foundations/recommendation-set-analysis.md`; `foundations/policy-over-prediction.md`; `PL-DEPLOY-EXPORT` (evidence/export + frozen-population discipline). The Reconciliation Completion Record (below in this file) is the durable reconciliation provenance. The first-experiment design/decomposition is `docs/design/pl-recipe-01-first-experiment-v1-design.md` (Category E supporting design artifact; design-only, not implementation/execution authorization). These link back to this `PL-RECIPE-01` identity and do not become a parallel registry.

### Pipeline state

**DISCOVERED → INTAKE → RECONCILED (2026-09-16).** Full §3 Strategic and §4 Architectural Reconciliation have been performed under Principal-authorized documentation preparation; see the **Reconciliation Completion Record for `PL-RECIPE-01`** below in this file. Neither `roadmap.md` nor `architecture-roadmap.md` was edited — both dispositions are "no change / refines existing," recorded durably in the completion record per methodology. The first-experiment design and implementation-readiness review are now complete (`docs/design/pl-recipe-01-first-experiment-v1-design.md`). **Durable stopping-state: first experiment fully specified and reviewed; implementation authorization pending.** This is not an `IMPLEMENTABLE` state; **experiment execution and all implementation remain NOT authorized**.
...
## Reconciliation Completion Record — `PL-RECIPE-01` (Wheelwright Recipes)

**Date:** September 16, 2026
**SYNC at reconciliation:** `05ada5869fa0bc8b4b862887d73321a277aba882` (intake commit `b198d5e` durably on accepted `main`; advancement `05ada58` reconciled and immaterial to this item)
**Status of this record:** §3 Strategic Reconciliation **has been performed** and §4 Architectural Reconciliation **has been performed**, under Principal-authorized documentation preparation. This is the observable completion record that advances `PL-RECIPE-01` to **RECONCILED**. **No implementation and no experiment execution is authorized by this record.** Reaching RECONCILED authorizes only the next-stage *design/decomposition*.
**Scope discipline:** deliberately reconciles the recipe concept against current authority and verified implementation truth; does **not** reopen the Principal-settled targeted dispositions (population boundary, three-arm experiment, Delta-Fence equivalence, Explorer-surface hypothesis, DSL closed branch) recorded in the intake above.

### Intake

Canonical identity: **`PL-RECIPE-01`** (this file) — the investigation/product identity for Wheelwright Recipes: named, versioned **decision lenses** (comparative-fitness treatments) over the post-admissibility, pre-winner-selection candidate population. Not a new subsystem/engine/service. Concept home `PL-DEPLOY`; the INTAKE record is above.

### Strategic disposition (§3, against `docs/roadmap.md`)

**Strengthens existing strategic direction; NO new Bet; NO roadmap change required.** Recipes carry no strategic content the LVT does not already own; the concept is evidence-gathering/implementation beneath `LVT-GOAL-CONSEQUENCES`.

- **Primary:** `LVT-BET-COMPENSATION` ("compensation relative to accepted capital consequence is a better measure of deployment quality than premium yield alone"), tested via `LVT-INIT-COMP-COMPARE` ("**Compare those measures against current ranking dimensions**") — the first three-arm experiment is that Initiative made concrete (near-verbatim mapping).
- **Strengthens:** `LVT-BET-ACCEPTABILITY` + `LVT-INIT-ACCEPT-GATES` — the population boundary is the "absolute acceptability before relative fitness" separation applied to contract selection; and `LVT-BET-EXPLANATION` + `LVT-INIT-EXPLAIN-TRADEOFFS` — structured disagreement with rejection reasons / contributing factors is comparative explanation.
- **No new Bet:** per roadmap change discipline ("Do not manufacture a new Bet merely because a new implementation idea exists") and the LVT vocabulary rule that implementation ideas belong beneath Bets. Recipes introduce a *mechanism*, not a new outcome Wheelwright seeks.
- **Supports a stated non-goal:** recipes are deliberately *plural lenses*, the opposite of a single "Wheelwright Score" — actively consistent with the roadmap "no scalar Deployment score" owned-capital non-goal.
- **Quality/differentiation lens (material here):** the differentiator is **Trustability** ("Can I trust what I'm seeing?"), expressed through inspectable rationale and legitimate disagreement — treated as an emerging cross-cutting differentiator, **not** a new Goal.
- **Future-only (NOT activated by the first experiment):** `LVT-BET-OUTCOME-LEARNING` + `LVT-INIT-LEARN-PRESERVE` become relevant only if recipes later influence real decisions (recipe versioning/provenance). The first experiment measures disagreement/proxy on a frozen board, not economic outcomes; this mapping is recorded to prevent premature wiring into outcome-learning.
- **`roadmap.md` not edited** — the "no change" conclusion is recorded here so future actors do not re-ask.

### Architectural disposition (§4, against `docs/architecture-roadmap.md`, ADRs, contract, and verified implementation)

**Refines existing architecture (AR5); introduces NO new engine, service, or competing authority; conforms to the ADR-013/015/016/017 authority spine and the frozen evidence contract; satisfies the Simplicity Constraint.**

- **AR5 ownership.** A recipe is the explicit, named realization of AR5's **comparative-fitness** stage ("what is comparatively preferable among survivors"). Verified implementation truth (`options-prototype/src/write-desk/recommend.ts`, `recommendPuts`): the current pipeline **implicitly fuses** acceptability and fitness — per contract it computes posture and keeps the highest-**score** contract per posture bucket (`bestActionable`/`bestEdge`/`bestWait`), then per symbol collapses to `best = bestActionable ?? bestEdge ?? bestWait` (**exactly one candidate per symbol**, execution-posture-first then score — *not* yield-first), and only then `rankByPolicy` ranks the already-collapsed set. A recipe **generalizes the stage that winner-selection currently hard-codes**. It lives in the Decision Engine (one of the ratified Four Engines) under `PL-DEPLOY`. **No fifth engine, no new service.**
- **Population boundary (verified gate chain).** "Post-admissibility" = surviving, in order: confirmed-absence check → admissible+eligible expirations (`isEligible` → `isSubjectAdmissible`, Issue #16 backend per-subject verdict, fail-closed while authority pending) → eligible DTE (`selectEligibleExpirations`) → admissible chain → contract-level admissible-delta + hard-no (zero-bid, zero-OI; wide-spread annotated, not excluded). The recipe population is the full set of admissible `(symbol, expiration, strike)` triples **before** the per-symbol collapse. This boundary is itself an AR5 expression and preserves the survival-vs-insufficiency rule (relative superiority insufficient; WAIT/insufficient-evidence preserved). "Admissible" is **not** a universal safety certificate — only survival of those specific gates.
- **Relationship to winner-selection (the durable boundary).** Current winner-selection is a *specific, implicit* AR5 fitness function; a recipe is a *general, explicit* one. Whether a recipe eventually **replaces / parameterizes / runs beside** the current collapse is left to decomposition (intake §3). **Hard constraint through implementation-readiness: winner-selection behavior does not change**, and the experiment's Accepted-Wheelwright arm must run the *real* collapse+rank path (not a re-sort of already-collapsed winners) or it measures the wrong thing.
- **Authority-spine conformance (ADR-013/015/016/017).** A recipe is a *consumer*: it composes/presents authoritative facts and produces interpretation (ordering + rationale); it must not re-encode facts (ADR-013), must not create/reconstruct provenance (ADR-015/016 — candidates already carry `evidenceProvenance`), and must not re-derive backend session/admissibility policy from lower-level signals (ADR-017 — the boundary sits downstream of the Issue-#16 verdict). **No competing authority.** The seam already exists in code: `exportGreeks` (raw provider greeks+IV+update-time) is attached to each candidate and explicitly "never influences the assessment/posture."
- **Evidence semantics stay owned by the evidence/domain layer.** `midIv` ≠ `smvVol` (never aliased/collapsed to generic `iv`; no local inversion); absence = `null`, never `0`; exact provider zero preserved; absolute IV ≠ IV richness. These are owned by the frozen snapshot contract v1 + `write-desk/option-greeks.ts`, which a recipe **reuses**. A recipe expresses intent *over* evidence; it never redefines evidence meaning.
- **AR4 and AR8 (mapped, mostly future).** Any *multi-Greek / normalized* recipe depends on AR4 consequence semantics (capital/scenario normalization) — a **future dependency NOT activated** by the first experiment (Delta-Constrained Yield uses only existing delta + existing yield); recorded to prevent premature dimensional Greek arithmetic. AR8's reproducible-temporal-substrate discipline **does** govern the first experiment now (frozen population, capture-not-refresh, no hindsight); recipe-decision-provenance under AR8 is future (only if recipes influence real decisions).
- **Explicit non-ownership (architectural non-goals).** Recipes do **not** own: evidence acquisition/semantics; backend admissibility/session authority; the acceptability gates themselves; provider-IV/export capability (`PL-DEPLOY-EXPORT`); new evidence or IV-history (`PL-EVID-MVPTA`); trade-shape admission (`PL-STRAT-01`); recommendation-engine placement (`PL-ARCH-06`, untouched); a scalar Deployment score; and — per Principal decision — generalized configuration/DSL machinery (closed branch).
- **`architecture-roadmap.md` not edited** — "refines AR5, introduces no new pressure not already recorded" is the conclusion, recorded here.

### Parking-lot disposition / mapping

- `PL-RECIPE-01` — **RECONCILED** (2026-09-16). Retained as the canonical Recipes investigation/product identity under `PL-DEPLOY`.
- Mappings retained (not re-owned): `PL-DEPLOY` (concept home / Decision composition), AR4/AR5/AR8 (architecture pressures), `PL-DEPLOY-EXPORT` (evidence substrate + frozen-population discipline; the §6 Greeks+IV slice is now committed on `main`), `PL-EVID-MVPTA` (distinct broad evidence/TA program), `PL-STRAT-01` (future strategy-shape ↔ recipe interaction only), `foundations/recommendation-set-analysis.md` (population-observation cousin / grouping-heuristic pattern to reuse), `docs/18-recommendation-vocabulary-review.md` (generalize existing `RankingPolicy`, do not build a parallel ranking system), `PL-ARCH-06` (placement pressure/evidence only). No new `PL-*` and no new LVT identity created. No defect double-booking.

### Why-state

This record plus the INTAKE record above are the durable provenance. Additional chronological why-state is preserved in `docs/journal/project-journal-3.md` (2026-09-16 reconciliation entry). The load-bearing insight worth not rediscovering: current winner-selection **destructively collapses each symbol's contract population to one contract before ranking**, which is exactly why a recipe must see the pre-collapse population and why the experiment's Accepted-Wheelwright arm must run the real collapse+rank path. The reconciliation was prepared by Kiro under Principal-authorized documentation preparation; the dispositions were established from repository text and verified implementation truth, not pre-written.

### Next authorized mode

**Experiment design / decomposition — COMPLETE.** The precise, hand-off-ready specification of the first three-arm experiment (frozen input artifact; the three arms; identical-population requirement; real-Wheelwright control behavior; Delta-Fence≡Delta-Constrained-Yield equivalence check; measurements; success/stop criteria; provenance/replay; expected outputs) and its implementation-readiness review (tests/acceptance evidence defined before any code) are recorded in `docs/design/pl-recipe-01-first-experiment-v1-design.md`. **Durable stopping-state: first experiment fully specified and reviewed; implementation authorization pending.** This is not an `IMPLEMENTABLE` state. **Experiment execution, recipe-engine construction, winner-selection changes, a Recipes page, generalized/DSL machinery, and any production behavior change remain NOT authorized.** The next canonical action is a separate Principal implementation-authorization decision. Product-surface (Explorer page) remains a strong-but-unresolved hypothesis pending experiment evidence.

### Pipeline state

**DISCOVERED → INTAKE → RECONCILED (2026-09-16); first experiment fully specified and reviewed, implementation authorization pending.** Not an `IMPLEMENTABLE` state. Next canonical action: a separate Principal implementation-authorization decision. Experiment execution and all implementation remain NOT authorized.
