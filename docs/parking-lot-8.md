...
2. **Cash Deployment — Prod v0** (second consumer, Sep 10) — two affordances, both **zero backend change**:
   ...
   **Completion truthfulness (working-software correction):** the re-read must run the FULL bounded backoff and NOT early-exit on "generation advanced" — this appliance publishes new generations near-continuously, so a generation bump does not mean the *targeted* symbols were re-observed; an early exit merged an unrelated publication and left the age stale. The per-row spinner tracks **visible-state convergence** (the row's own `acquiredAtMs` advancing past the click-time baseline), not request completion; if acquired but the visible age does not advance within a bounded wait (e.g. a provider-cache hit inside the 90s chain-cache window), it stops **indeterminate**, never a clean success. Promoted to `foundations/visual-design-principles.md` principle 12.
