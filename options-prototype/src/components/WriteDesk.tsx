...
export function WriteDesk() {
  ...
  return (
    <div className={`write-desk${selectedCandidate ? " wd-with-drawer" : ""}`}>
      ...
      {/* ═══ BAND 2: Operating Context (trust + coverage) ═══ */}
        ...
        <div className="wd-band wd-band-context">
          {trustIndicator && (
            <span className={`wd-trust wd-trust-${trustIndicator.color}`} title={`Generation: ${evidenceMeta?.generation}`}>
              ...
              {" · "}{trustIndicator.covered}/{trustIndicator.universe} resolved
              {" · "}{putFunnel ? putFunnel.eligible : "—"} opportunities
              {" · "}{trustIndicator.freshnessLabel}
              {trustIndicator.activity === "updating" && " · Updating"}
            </span>
          )}
          {!trustIndicator && !evidenceMeta && (
            <span className="wd-trust wd-trust-yellow">
              <span className="wd-trust-dot">●</span>
              Connecting to evidence service...
            </span>
          )}
          <details className="wd-portfolio-disclosure">
            <summary className="wd-portfolio-disclosure-summary">Portfolio detail</summary>
            <PortfolioDetail snapshot={snapshot} pendingIntents={pendingIntents} onIntentResolve={(id, status) => { updatePendingIntent(id, status); setPendingIntents(loadWorkingIntents()); }} />
          </details>
        </div>
      )}
      ...
      {snapshot && snapshot.readiness.status === "READY" && (scanTimestamp || evidenceMeta) && (
        <section className="wd-board">
          {/* Board header: title + controls + infographic */}
              ...
              <div className="wd-board-controls">
                <label className="wd-control">
                  ...
                  <select value={policy.ranking.mode} onChange={(e) => { const updated = { ...policy, ranking: { ...policy.ranking, mode: e.target.value as any } }; setPolicy(updated); handleReRecommend(updated); updateWorkspace({ writeDeskRankingMode: e.target.value }); }} className="wd-control-select">
                    <option value="execution_first">Execution</option>
                    <option value="balanced">Balanced</option>
                    <option value="yield_first">Yield</option>
                    <option value="capital_efficiency">Capital Eff.</option>
                  </select>
                </label>
                <label className="wd-control">
                  Δ
